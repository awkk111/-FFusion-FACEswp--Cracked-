...
def create_occlusion_mask(crop_vision_frame : VisionFrame) -> Mask:
	...
	occlusion_mask = (cv2.GaussianBlur(occlusion_mask.clip(0, 1), (0, 0), 5).clip(0.5, 1) - 0.5) * 4
...
def create_region_mask(crop_vision_frame : VisionFrame, face_mask_regions : List[FaceMaskRegion]) -> Mask:
	...
	region_mask = (cv2.GaussianBlur(region_mask.clip(0, 1), (0, 0), 15).clip(0.5, 1) - 0.5) * 2
	return region_mask
...
def create_mouth_mask(face_landmark_68 : FaceLandmark68) -> Mask:
	...
	mouth_mask = cv2.erode(mouth_mask.clip(0, 1), numpy.ones((5, 5)))
	mouth_mask = cv2.GaussianBlur(mouth_mask, (0.5, 0.5), sigmaX = 1, sigmaY = 1) * 4
	return mouth_mask
